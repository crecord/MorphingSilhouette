...
void morph::setup(string pathToImages, int x, int y){
  ...
  //   blurX.load("shadersGL3/shaderBlurX");
    ...
    drawTrailingBlurY.allocate(ofGetWidth(), ofGetHeight() );
...
ofPath morph::pathToPath(ofPath pth){
   ...
   // ofPolyline cur;
    ...
    drawNoColor.allocate(ofGetWidth(), ofGetHeight(),GL_RGBA);
    ofPixels pixels;
    ...
    drawTrailing.readToPixels(pixels);

    colorImg.setFromPixels(pixels);
    //colorImg.blurGaussian();
    grayImg = colorImg;
    //grayImg.blur();
    grayImg.threshold(globalFilterThresh);
    ...
    if(contourFinder.nBlobs > 0){
        ...
        for( int i=0; i< contourFinder.blobs[0].pts.size(); i++){
            ofVec2f pos= contourFinder.blobs[0].pts.at(i);
            pos = ofVec2f(pos.x-ofGetWidth()/2,pos.y-ofGetHeight()/2);
            cur.addVertex(pos);
        }
        cur.setClosed(true);
    ...
    vector<ofPoint> pnts = cur.getVertices();
    
    for( int i = 0; i < pnts.size(); i++) {
        if(i == 0) {
            toReturn.newSubPath();
            toReturn.moveTo(pnts.at(i));
        } else {
            toReturn.curveTo(pnts.at(i));
        }
    }
    toReturn.close();
    toReturn.setPolyWindingMode(OF_POLY_WINDING_POSITIVE);
